...
-- Para converter com FORMAT: FORMAT (DataNascimento, 'dd/MM'yyy')
...
/* Liste nome, cpf, data de nascimento (em formato DD/MM/YYYY), email, cidade aonde mora e a UF em ordem alfabética de nomes;*/

SELECT
A.Nome, 
A.CPF,
CONVERT (VARCHAR, A.DataNascimento, 103) AS 'Data Nascimento',
A.Email,
C.Nome AS Cidade,
U.UF AS UF
FROM Alunos A
...
/* Liste nome e quantos cursos cada aluno comprou em ordem alfabética; */

SELECT
A.Id,
A.Nome,
COUNT (AC.CursoId) AS 'Quantidade de Cursos'
FROM Alunos A
...
/* Usando o Select anterior, exiba também o total de mensalidade que cada aluno paga. O total é a soma das mensalidades dos cursos que o aluno possui (dica: terá que fazer join com a tabela Cursos); */

SELECT
A.Nome,
COUNT (AC.CursoId) AS 'Quantidade de Cursos',
SUM (C.Mensalidade) AS 'Soma das Mensalidades'
FROM Alunos A
LEFT JOIN AlunosCursos AC ON A.Id = AC.AlunoId	
...
/* Liste quantos alunos cada curso possui. Considere que pode haver cursos sem aluno! */
...
SELECT
C.Nome,
COUNT (AC.CursoId) AS 'Quantidade de Alunos'
FROM Cursos C
LEFT JOIN AlunosCursos AC ON C.Id = AC.CursoId
GROUP BY C.Id, C.Nome


/* Exiba o top 3 cursos mais comprados. */
